...
export default function Reports({ records = [] }) {
  ...
  const downloadIndividualPDF = (record) => {
    ...
    doc.setFontSize(24); doc.setTextColor(30, 58, 138); doc.setFont(undefined, 'bold'); doc.text("OFFICIAL SERVICE INTAKE", 14, 20);
    doc.setFontSize(10); doc.setTextColor(100, 116, 139); doc.setFont(undefined, 'normal');
    doc.text(`Document Ref: #${record.id.toString().slice(-8).toUpperCase()} | Timestamp: ${record.date}`, 14, 28);

    doc.setFontSize(11); doc.setTextColor(15, 23, 42); doc.setFont(undefined, 'bold');
    doc.text("CLIENT DETAILS", 14, 50); doc.text("VEHICLE IDENTIFICATION", 110, 50);
    doc.setFont(undefined, 'normal'); doc.setFontSize(10); doc.setTextColor(71, 85, 105);
    doc.text(`Name: ${record.fleetOwnerName}\nCompany: ${record.companyName || 'N/A'}\nPhone: ${record.fleetOwnerContact}`, 14, 58);
    doc.text(`Registration: ${record.vehicleNumber}\nRequested Job: ${record.jobType}`, 110, 58);

    const batteryStat = getBatteryStatus(record.batteryHealth);
    const damageStat = getDamageAnalysis(record.exteriorCondition);
    
    doc.setDrawColor(226, 232, 240); doc.setFillColor(248, 250, 252);
    doc.roundedRect(14, 80, pageWidth - 28, 25, 3, 3, 'FD');
    doc.setFontSize(10); doc.setTextColor(15, 23, 42); doc.setFont(undefined, 'bold');
    doc.text("SYSTEM AUTOMATED DIAGNOSTICS", 18, 88);
    doc.setFont(undefined, 'normal'); doc.setFontSize(9); doc.setTextColor(71, 85, 105);
    doc.text(`Battery Status: ${batteryStat.label}`, 18, 95);
    doc.text(`Body Assessment: ${damageStat.reason} -> ${damageStat.action}`, 18, 101);

    autoTable(doc, {
      startY: 115, theme: 'grid', head: [['Physical Inspection Check', 'Recorded Status']],
      body: [
    ...
    doc.setFont(undefined, 'bold'); doc.setTextColor(15, 23, 42); doc.text("CLIENT REPORTED ISSUES", 14, finalY);
    ...
    if (record.extraNotes && record.extraNotes.length > 0) {
      doc.setFont(undefined, 'bold'); doc.setTextColor(15, 23, 42); doc.text("POST-INSPECTION MECHANIC REMARKS", 14, finalY);
    ...
    doc.setFontSize(8); doc.setTextColor(100, 116, 139); doc.text("Terms: The client authorizes the diagnostic work detailed above.", 14, sigY - 10);
    doc.setDrawColor(148, 163, 184); doc.line(14, sigY, 70, sigY); doc.line(140, sigY, 196, sigY);
    doc.setFontSize(9); doc.setTextColor(15, 23, 42); doc.text("Service Advisor Signature", 14, sigY + 5); doc.text("Client Authorization", 140, sigY + 5);
  ...
  return (
    <div className="max-w-[90rem] mx-auto space-y-6 sm:space-y-8 animate-in fade-in duration-500 pb-12 px-4 sm:px-6 xl:px-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button onClick={downloadMasterPDF} className="w-full md:w-auto flex justify-center items-center gap-2 bg-indigo-600 text-white hover:bg-indigo-700 px-6 py-3 rounded-xl font-bold text-sm transition-all duration-300 shadow-lg hover:-translate-y-0.5 active:scale-95">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative">
        ...
        {/* Sticky Filters */}
        <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-100 p-4 sm:p-5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center shadow-sm">
