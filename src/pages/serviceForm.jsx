    ...
    // Helper component for text inputs to keep the code clean and consistent
    const InputGroup = ({ label, name, value, onChange, error, placeholder, description, type = "text" }) => (
      <div className="flex flex-col gap-1.5 mb-5 sm:mb-6">
        ...
        {/* Rich helper text to guide the user */}
          ...
          <input
            ...
            className={`w-full px-4 py-3 rounded-xl border-2 transition-all outline-none text-slate-700 text-sm sm:text-base
              ${error ? "border-red-300 bg-red-50 focus:border-red-500 focus:ring-4 focus:ring-red-100" 
                     : "border-slate-200 bg-slate-50 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50"}`}
          />
        </div>
      </div>
    );
    ...
    export default function ServiceForm({ addRecord }) {
      ...
      const onSubmit = (e) => {
        ...
        if (validate()) {
          ...
          // Simulating a network delay to show the loading state
          setTimeout(() => {
      ...
      // ----------------------------------------------------------------------
      // SUCCESS SCREEN UI (What the user sees after submitting)
      // ----------------------------------------------------------------------
      if (isSubmitted) {
        return (
          <div className="max-w-xl mx-auto mt-12 py-12 px-6 bg-white rounded-3xl border border-slate-200 shadow-xl text-center animate-in zoom-in-95 duration-500">
            ...
            <p className="text-slate-500 mb-8 max-w-md mx-auto leading-relaxed">
              The service request for vehicle <span className="font-bold text-slate-800 bg-slate-100 px-2 py-1 rounded">{formData.vehicleNumber}</span> has been securely saved to the database. You can now track its status in the Reports dashboard.
            </p>
            <button 
              onClick={handleReset}
              className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-2 mx-auto"
            >
      ...
      return (
        <div className="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-500">
          ...
          {/* Friendly Introduction Header */}
          <div className="bg-blue-600 rounded-2xl p-6 sm:p-8 mb-6 sm:mb-8 text-white shadow-xl shadow-blue-100">
            <h1 className="text-2xl sm:text-3xl font-bold mb-3">New Service Request</h1>
            <p className="opacity-90 text-blue-50 text-sm sm:text-base leading-relaxed">
              Welcome to the intake portal. Please fill in the details below accurately. The helper text under each box will guide you on exactly what information our mechanics need.
            </p>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-6 sm:space-y-8">
            ...
            <div className="bg-white p-5 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
              <div className="flex items-start gap-4 mb-6 sm:mb-8 pb-5 border-b border-slate-100">
                <div className="w-8 h-8 sm:w-10 sm:h-10 shrink-0 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-sm sm:text-base mt-1">1</div>
                <div>
                  <h2 className="text-lg sm:text-xl font-bold text-slate-800">Vehicle & Owner Details</h2>
                  <p className="text-xs sm:text-sm text-slate-500 mt-1 leading-relaxed">
                    We need to know exactly which vehicle is being serviced and who to contact when the job is complete.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                <InputGroup 
                  label="Vehicle Registration Number" name="vehicleNumber" 
                  value={formData.vehicleNumber} onChange={handleChange} error={errors.vehicleNumber}
                  placeholder="e.g. MH-12-AB-1234" 
                  description="Enter the exact letters and numbers found on the vehicle's license plate."
                />
                <InputGroup 
                  ...
                  description="If this vehicle belongs to a corporate fleet, enter the company name. Leave blank if personal."
                />
                <InputGroup 
                  ...
                  description="The full legal name of the person responsible for authorizing this service."
                />
                <InputGroup 
                  ...
                  description="Provide a direct 10-digit mobile number so our team can call for approvals." 
                  type="tel"
                />
              </div>
              <InputGroup 
                ...
                description="We will send the digital PDF service report, billing invoices, and updates to this email." 
                type="email"
              />
            </div>
            ...
            <div className="bg-white p-5 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
              <div className="flex items-start gap-4 mb-6 sm:mb-8 pb-5 border-b border-slate-100">
                <div className="w-8 h-8 sm:w-10 sm:h-10 shrink-0 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-sm sm:text-base mt-1">2</div>
                <div>
                  <h2 className="text-lg sm:text-xl font-bold text-slate-800">Service Requirements</h2>
                  <p className="text-xs sm:text-sm text-slate-500 mt-1 leading-relaxed">
                    Tell us what kind of work needs to be done so we can assign the right mechanic to the job.
                  </p>
                </div>
              </div>
              ...
              <div className="mb-6 sm:mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {[
                    { 
                      id: "Quick Service", 
                      title: "Quick Service (Minor)",
                      desc: "Ideal for regular maintenance: Oil changes, fluid top-ups, filter replacements, and standard visual checks. Usually takes 1-2 hours." 
                    },
                    { 
                      id: "General Service", 
                      title: "General Service (Major)",
                      desc: "Comprehensive checkup: Deep engine diagnostics, part replacements, suspension work, and full body check. Usually takes 1-2 days." 
                    }
                  ].map(type => (
                    <button
                    ...
                    >
                      <p className={`font-bold text-sm sm:text-base mb-1 ${formData.jobType === type.id ? "text-blue-700" : "text-slate-700"}`}>{type.title}</p>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500 mb-2 leading-relaxed">
                  Please describe the problem in your own words. Mention any strange noises (e.g., "squeaking brakes"), leaks under the car, or dashboard warning lights that are currently on.
                </p>
                <textarea 
                  name="issueDescription" rows="5" 
                  value={formData.issueDescription} onChange={handleChange} 
                  placeholder="e.g., The vehicle struggles to start in the morning and there is a rattling sound coming from the rear right wheel..."
                  className="w-full p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-sm focus:border-blue-500 focus:bg-white transition-all outline-none resize-y"
                />
              </div>
            </div>

            {/* STEP 3: CONDITIONAL CHECKLIST (Only visible if General Service is selected) */}
            {formData.jobType === "General Service" && (
              <div className="bg-slate-50 p-5 sm:p-8 rounded-3xl border-2 border-dashed border-slate-300 space-y-6 sm:space-y-8 animate-in slide-in-from-top-4 duration-300">
                
                 <div className="text-center mb-2">
                    <span className="bg-slate-200 text-slate-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">Mandatory Section</span>
                    ...
                    <p className="text-xs text-slate-500 mt-2 max-w-lg mx-auto leading-relaxed">
                      Because you selected a Major General Service, our mechanics require a baseline health check of the vehicle before it enters the garage. Please inspect the following:
                    </p>
                 </div>
                
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
                    {/* Exterior Condition Radio Buttons */}
                    <div className="space-y-3 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                      <div className="flex flex-col gap-1 mb-2">
                        ...
                        <p className="text-[11px] text-slate-500 leading-relaxed">Walk around the vehicle. Are there any visible physical damages to the doors, bumpers, or glass?</p>
                      </div>
                      <div className="flex flex-col gap-2.5">
                        {[
                          ...
                          <label key={item.val} className={`flex items-start gap-3 p-3.5 border rounded-xl cursor-pointer transition-colors ${formData.exteriorCondition === item.val ? "bg-blue-50 border-blue-200" : "bg-white border-slate-200 hover:bg-slate-50"}`}>
                            <input type="radio" name="exteriorCondition" value={item.val} checked={formData.exteriorCondition === item.val} onChange={handleChange} className="w-4 h-4 mt-0.5 text-blue-600" />
                    ...
                    <div className="space-y-3 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                      <div className="flex flex-col gap-1 mb-2">
                        ...
                        <p className="text-[11px] text-slate-500 leading-relaxed">Observe the vehicle's paint. Does it require polishing, matching, or full repainting?</p>
                      </div>
                      <div className="flex flex-col gap-2.5">
                        {[
                          ...
                          <label key={item.val} className={`flex items-start gap-3 p-3.5 border rounded-xl cursor-pointer transition-colors ${formData.paintCondition === item.val ? "bg-blue-50 border-blue-200" : "bg-white border-slate-200 hover:bg-slate-50"}`}>
                            <input type="radio" name="paintCondition" value={item.val} checked={formData.paintCondition === item.val} onChange={handleChange} className="w-4 h-4 mt-0.5 text-blue-600" />
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-slate-700">{item.label}</span>
                              <span className="text-[10px] text-slate-500">{item.sub}</span>
                            </div>
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 pt-4">
                    <InputGroup 
                      ...
                      placeholder="e.g. 85"
                      description="Check the vehicle's dashboard diagnostic screen or provide the last known battery health reading (0-100)." 
                    />
                    <InputGroup 
                      label="Current Tyre Pressure (PSI)" name="tyrePressure" 
                      value={formData.tyrePressure} onChange={handleChange} 
                      placeholder="e.g. 32 PSI"
                      description="Note the current air pressure in the front tyres. You can find the recommended PSI on the inside of the driver's door jamb." 
                    />
                 </div>
              </div>
            )}
            ...
            <button 
              ...
              className={`w-full text-white py-4 sm:py-5 rounded-2xl font-bold text-base sm:text-lg shadow-2xl transition-all flex items-center justify-center gap-3 mt-4 active:scale-[0.98]
                ${isSubmitting ? "bg-slate-400 cursor-not-allowed shadow-none" : "bg-slate-900 hover:bg-slate-800 hover:-translate-y-1 hover:shadow-slate-300"}`}
            >
              {isSubmitting ? (
                <>Saving to Database... <Loader2 size={20} className="animate-spin" /></>
              ) : (
